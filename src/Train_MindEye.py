...
train_url = "{" + f"{args.data_path}/webdataset_avg_split/train/train_subj0{args.subj}_" + "{0..17}.tar," + f"{args.data_path}/webdataset_avg_split/val/val_subj0{args.subj}_0.tar" + "}"
...
if not args.prior:# 只训练 voxel2clip 部分，冻结扩散先验模型。
...
if args.n_samples_save > 0 and args.hidden:
    ...
    try:
        vd_pipe = VersatileDiffusionDualGuidedPipeline.from_pretrained(args.vd_cache_dir).to('cpu')
    except:
...
if local_rank == 0 and args.wandb_log:
    # only use main process for wandb logging
    import wandb
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, coco) in enumerate(train_dl):
        with torch.cuda.amp.autocast():
            ...
            if distributed:
                clip_voxels, clip_voxels_proj = diffusion_prior.module.voxel2clip(voxel)
            else:
            ...
            if args.prior:
                if distributed:
                    loss_prior, aligned_clip_voxels = diffusion_prior.module(
                        text_embed=clip_voxels, image_embed=clip_target)
                    aligned_clip_voxels /= diffusion_prior.module.image_embed_scale
                else:
                    loss_prior, aligned_clip_voxels = diffusion_prior(
                        text_embed=clip_voxels, image_embed=clip_target)
            ...
            #         temp=.006, perm=perm, betas=betas, select=select)
            # else:
    ...
    for val_i, (voxel, image, coco) in enumerate(val_dl):
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                if args.prior:
                    if distributed:
                        val_loss_prior, aligned_clip_voxels = diffusion_prior.module(
                            text_embed=clip_voxels, image_embed=clip_target)
                        aligned_clip_voxels /= diffusion_prior.module.image_embed_scale
                    else:
                        val_loss_prior, aligned_clip_voxels = diffusion_prior(
                            text_embed=clip_voxels, image_embed=clip_target)
                        aligned_clip_voxels /= diffusion_prior.image_embed_scale
                else:
                    aligned_clip_voxels = clip_voxels
                ...
                #         clip_voxels_norm, clip_target_norm,
                #         temp=.006, perm=None, betas=None, select=None)
                # else:
